...
class PaintApp:
    def __init__(self, root):
        ...
        # 创建重做按钮
        self.redo_button = tk.Button(self.toolbar, text="重做", command=self.redo)
        ...
        self.shape_menu = tk.OptionMenu(self.toolbar, self.shape_var, "Freehand", "Line", "Rectangle", "Oval", "Text")
    ...
    def paint(self, event):
        ...
        if shape == "Freehand":
            ...
            if self.old_x and self.old_y:
                ...
                self.canvas.create_line(self.old_x, self.old_y, event.x, event.y, fill=color, width=brush_size, tags="preview")
        elif shape == "Rectangle":
            if self.old_x and self.old_y:
                self.canvas.delete("preview")
                self.canvas.create_rectangle(self.old_x, self.old_y, event.x, event.y, outline=color, width=brush_size, tags="preview")
        elif shape == "Oval":
            if self.old_x and self.old_y:
                self.canvas.delete("preview")
                self.canvas.create_oval(self.old_x, self.old_y, event.x, event.y, outline=color, width=brush_size, tags="preview")
        elif shape == "Text":
            self.canvas.create_text(event.x, event.y, text=self.text_entry.get(), fill=color, font=("Arial", brush_size * 5))

    def reset(self, event):
        shape = self.shape_var.get()
        if shape in ["Line", "Rectangle", "Oval"]:
            self.canvas.delete("preview")
            brush_size = self.brush_size.get()
            color = "white" if self.eraser_on else self.paint_color
            if shape == "Line":
                ...
                self.canvas.create_rectangle(self.old_x, self.old_y, event.x, event.y, outline=color, width=brush_size)
                self.draw.rectangle([self.old_x, self.old_y, event.x, event.y], outline=color, width=brush_size)
            elif shape == "Oval":
                self.canvas.create_oval(self.old_x, self.old_y, event.x, event.y, outline=color, width=brush_size)
                self.draw.ellipse([self.old_x, self.old_y, event.x, event.y], outline=color, width=brush_size)
        self.old_x = None
        self.old_y = None
